...
public class BaseController extends BaseUtil {
    ...
    private PageData getPageJson(HttpServletRequest request, Object o) {
        ...
        HttpServletRequest request3 = this.getRequest();
...
//    }
     ...
     * 更改参数形式
     */
    protected PageData getParamsFormat(String format) {
        return putUserInfoFormat(this.getRequest(), format);
    }

    /**
     * 自定义参数形式，而不是固定get为xml，post/delete为json
     * @param request
     * @param format
     * @return
     */
    private PageData putUserInfoFormat(HttpServletRequest request, String format) {
        String userToken = request.getHeader("token");
        //将用户信息放进pageData
        PageData pageData = null;
