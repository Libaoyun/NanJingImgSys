...
public class DictionaryServiceImpl implements DictionaryService {
    ...
    public void updateDictionaryInfo(PageData pd) {
        ...
        List<PageData> enumList = (List<PageData>) baseDao.findForList("DictionaryMapper.queryDictionariesList", pd);
        if(!CollectionUtils.isEmpty(enumList)){
            for(PageData dicEnum : enumList){
                if(!dicEnum.getString("id").equals(pd.getString("id'"))){
                    throw new MyException(ErrorCodeEnum.DIC_ENUM_EXIST.desc());
                }
            }
        ...
        //判断枚举值是否重复
        List<PageData> uqDictionaryList = (List<PageData>) baseDao.findForList("DictionaryMapper.selectDictionaryList", pd);
        if(!CollectionUtils.isEmpty(uqDictionaryList)){
            for(PageData dic : uqDictionaryList){
                if(dic.getString("dicEnumName").equals(pd.getString("dicEnumName")) &&
                        dic.getString("dicTypeId").equals(pd.getString("dicTypeId")) &&
                        !dic.getString("id").equals(pd.getString("id"))){
                    throw new MyException(ErrorCodeEnum.DATA_HAS_EXIST.desc());
                }
            }
        }
    ...
    public List<PageData> queryDictionaryType() {
        ...
        List<PageData> dictTypeList = (List<PageData>) baseDao.findForList("DictioinaryTypeMapper.selectList");
    ...
    public void saveDictionType(PageData pd) {
        ...
        //先查询新增的数据在数据库中是否已经存在
        PageData data = (PageData) baseDao.findForObject("DictioinaryTypeMapper.selectDictionaryTypeByTypeIdOrTypeName",pd);
        if(data != null){
            throw new MyException(ErrorCodeEnum.DATA_HAS_EXIST.desc());
        }

        baseDao.insert("DictioinaryTypeMapper.insertDictionTypeData",pd);
    ...
    public void modifyDictionType(PageData pd) {
        ...
        List<PageData> dictTypeList = (List<PageData>) baseDao.findForList("DictioinaryTypeMapper.selectList");
        for(PageData dictType : dictTypeList){
            if(dictType.getString("dicTypeName").equals(pd.getString("dicTypeName"))){
                //类别的id 存在，那么提示数据已经存在
                throw new MyException(ErrorCodeEnum.DATA_HAS_EXIST.desc());
            }
        }

        baseDao.update("DictioinaryTypeMapper.updateDictionTypeData",pd);
    ...
    public void removeDictionType(PageData pd) {
        ...
        if(!CollectionUtils.isEmpty(codeList)){
            //删除字典类型
            baseDao.batchDelete("DictioinaryTypeMapper.deleteDictionaryType",codeList);

            //筛选对应的枚举值
            baseDao.batchDelete("DictioinaryTypeMapper.deleteDictionary",codeList);
        }
