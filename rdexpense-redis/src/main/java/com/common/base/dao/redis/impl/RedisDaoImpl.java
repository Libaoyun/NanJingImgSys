...
public class RedisDaoImpl extends AbstractBaseRedisDao<Object, Object> implements RedisDao {
    ...
    @Override
    public boolean vSetIncrement(Object key, Long value, Long expireTime) {
        boolean result = false;
        try {
            ValueOperations<Object, Object> operations = redisTemplate.opsForValue();
            operations.increment(key, value);
	...
	@Override
	public boolean vSetAt(Object key, Object value, Date date) {
		boolean result = false;
        try {
            ValueOperations<Object, Object> operations = redisTemplate.opsForValue();
            operations.set(key, value);
    ...
    @Override
    public Set<String> getScan(String key) {
        Set<String> keys = new HashSet<>();
        this.scan(key, item -> {
            //符合条件的key
            String getKey = new String(item, StandardCharsets.UTF_8);
            keys.add(getKey);
        });
        return keys;
    }
