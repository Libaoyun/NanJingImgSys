        ...
        <div class="content">
            <el-form ref="routerMenuForm"  :model="routerMenuInfo" :rules="routerMenuFormRule" size="mini" label-position="left" label-width="120px">
                ...
                <el-form-item label="是否列表:" prop="pathRouting">
                    <el-select v-model="routerMenuInfo.pathRouting" @change="routerMenuInfo.pathRouting==1? '' : $set(routerMenuInfo,'comButton',routerMenuInfo.comButton.split(','))">
                        <el-option label="是" value="0"></el-option>
                        <el-option label="否" value="1"></el-option>
                    </el-select>
                </el-form-item>
...
export default class extends tableMixin {
    ...
    submitBtn() {
        this.$refs['routerMenuForm'].validate((valid) => {
          if (valid) {
            this.$emit('editRouterMenu',this.routerMenuInfo);
          } else {
            return false;
          }
        });
