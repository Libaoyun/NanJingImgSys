<!--
* @description 
* @fileName create.vue
* @author Miaoxy
* @date 2021/11/24 21:04:31
!-->
<template>
  <div style="padding-bottom: 46px;" class="contract">
    <card-global
        class="global-card-default"
        cardTitle="研发项目进展报告-新建"
        style="height:300px"
    >
      <div style="margin-top:20px;">
        <el-form
            ref="doForm"
            :inline="true"
            :rules="formRules"
            :model="baseInfo"
            size="mini"
            label-position="right"
            label-width="100px"
        >
          <el-form-item label="单据编号" prop="conId">
            <el-input
                v-model="baseInfo.conId"
                placeholder="自动生成"
                disabled
            ></el-input>
          </el-form-item>
          <el-form-item label="创建人" prop="createUser">
            <el-input v-model="baseInfo.createUser" disabled></el-input>
          </el-form-item>
          <el-form-item label="创建时间" prop="createTime">
            <el-input
                :value="baseInfo.createTime | formatTime('yyyy-MM-dd')"
                disabled
            ></el-input>
          </el-form-item>
          <el-form-item label="项目名称" prop="orgName">
            <el-input
                v-model="baseInfo.orgName"
                placeholder="请输入项目名称"
            ></el-input>
          </el-form-item>
          <el-form-item label="所属单位名称" prop="companyName">
            <el-select v-model="baseInfo.companyName" placeholder="自动带入">
              <el-option
                  v-for="item in baseInfo.companyName"
                  label="启用"
                  value="item"
                  :key="item"
              ></el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="项目负责人" prop="leader">
            <el-input
                :value="baseInfo.leader"
                placeholder="自动带入"
            ></el-input>
          </el-form-item>
          <el-form-item label="负责人岗位" prop="station">
            <el-input
                :value="baseInfo.station"
                placeholder="自动带入"
            ></el-input>
          </el-form-item>
          <el-form-item label="联系电话" prop="tel">
            <el-input :value="baseInfo.tel" placeholder="自动带入"></el-input>
          </el-form-item>
          <el-form-item label="交底书编制人" prop="disclosureCompiler">
            <el-input
                :value="baseInfo.disclosureCompiler"
                placeholder="点击弹窗选择人员"
            ></el-input>
          </el-form-item>
          <el-form-item label="编制日期" prop="comTime">
            <el-date-picker
                type="date"
                placeholder="点击选择"
                v-model="baseInfo.comTime"
                value-format="yyyy-MM-dd"
            >
            </el-date-picker>
          </el-form-item>
          <el-form-item label="编制年度" prop="comYear">
            <el-date-picker
                type="date"
                placeholder="点击选择"
                v-model="baseInfo.comYear"
                value-format="yyyy"
            >
            </el-date-picker>
          </el-form-item>
          <el-form-item label="编制季度" prop="comQuarter">
            <el-select v-model="baseInfo.comQuarter" placeholder="请选择">
            </el-select>
          </el-form-item>
        </el-form>
      </div>
    </card-global>
    <card-global
        class="global-card-default"
        cardTitle="交底书内容信息"
        style="height:1600px"
    >
      <el-form
          ref="doForm"
          :inline="false"
          :rules="formRules"
          :model="baseInfo"
          size="mini"
          label-position="left"
          label-width="100%"
      >
        <el-form-item
            label="
            一、本期主要研发内容季关键（核心）技术及主要创新点：
            "
            prop="coreTechnology"
        >
          <el-input
              :rows="10"
              v-model="baseInfo.coreTechnology"
              placeholder="内容不得少于200字"
              type="textarea"
              style="width:1000px;"
              resize="none"
          ></el-input>
        </el-form-item>
        <el-form-item
            label="
            二、本期工作（进度）安排：
            "
            prop="schedule"
        >
          <el-input
              :rows="10"
              v-model="baseInfo.schedule"
              placeholder="内容不得少于200字"
              type="textarea"
              style="width:1000px;"
              resize="none"
          ></el-input>
        </el-form-item>
        <el-form
            prop="concreteArrangement"
            ref="doForm"
            :inline="false"
            :rules="formRules"
            :model="baseInfo"
            size="mini"
            label-position="left"
            label-width="100%"
        >
          <el-form-item
              label="
            三、本期研究开发具体安排：
            "
          ></el-form-item>

          <el-form-item
              label="
            （一）人工安排：
            "
              prop="manual"
          >
            <el-input
                :rows="10"
                v-model="baseInfo.manual"
                placeholder="内容不得少于200字"
                type="textarea"
                style="width:1000px;"
                resize="none"
            ></el-input>
          </el-form-item>
          <el-form-item
              label="
            （二）研发仪器设备安排：
            "
              prop="equipment"
          >
            <el-input
                :rows="10"
                v-model="baseInfo.equipment"
                placeholder="内容不得少于200字"
                type="textarea"
                style="width:1000px;"
                resize="none"
            ></el-input>
          </el-form-item>
          <el-form-item
              label="
            （三）材料、燃料、动力费用投入安排：
            "
              prop="cost"
          >
            <el-input
                :rows="10"
                v-model="baseInfo.cost"
                placeholder="内容不得少于200字"
                type="textarea"
                style="width:1000px;"
                resize="none"
            ></el-input>
          </el-form-item>
          <el-form-item
              label="
            （四）模具、工艺装备开发及制造及设备调试安排：
            "
              prop="debug"
          >
            <el-input
                :rows="10"
                v-model="baseInfo.debug"
                placeholder="内容不得少于200字"
                type="textarea"
                style="width:1000px;"
                resize="none"
            ></el-input>
          </el-form-item>
          <el-form-item
              label="
            （五）其他：
            "
              prop="other"
          >
            <el-input
                :rows="10"
                v-model="baseInfo.other"
                placeholder="内容不得少于200字"
                type="textarea"
                style="width:1000px;"
                resize="none"
            ></el-input>
          </el-form-item>
        </el-form>
      </el-form>
    </card-global>
    <card-global
        class="global-card-default"
        cardTitle="附件上传"
        style="padding-top: 0px;height: 800px"
    >
      <el-card
          body-style="border:2px dashed #ddd;
          height:200px; 
          display:flex; 
          justify-content:center; 
          align-items:center;
          padding-bottom:20px;"
          @click="handleUpload()"
      >
        <!-- 图片暂时缺省 -->
        <img src="" alt=""/>
        <span>点击或拖拽进行文件上传</span>
      </el-card>
      <el-button
          style="margin-top:20px"
          :disabled="isUpdata"
          @click="downFiles()"
      ><span>批量下载</span>
      </el-button>
      <upload-approval-global
          type="create"
          ref="uploadApprovalGlobal"
          :fileList="baseInfo.attachmentList"
      ></upload-approval-global>
    </card-global>
    <div class="global-fixBottom-actionBtn">
      <el-button size="mini" @click="backBtn">返回</el-button>
      <loading-btn
          size="mini"
          type="primary"
          @click="saveBtn"
          :loading="loadingBtn"
      >保存
      </loading-btn
      >
    </div>
  </div>
</template>

<script>
import {Component, Vue, Watch} from 'vue-property-decorator';
import tableMixin from '@/mixins/tableMixin';
import {checkForm} from '@/utils/index';

@Component({
  name: 'contractNew',
  components: {},
})
export default class extends tableMixin {
  loadingBtn = 0;
  baseInfo = this.getBaseInfo();
  formRules = {
    orgName: [
      {required: true, message: '请输入项目名称', trigger: 'change'},
      {max: 30, message: '项目名称不超过30个字符', trigger: 'change'},
    ],
    tel: [
      {required: true, message: '请输入联系电话', trigger: 'change'},
      {max: 20, message: '联系电话不超过20个字符', trigger: 'change'},
    ],
    disclosureCompiler: [
      {required: true, message: '请输入交底书编制人', trigger: 'change'},
    ],
    comTime: [{required: true, message: '请输入编制日期', trigger: 'change'}],
    comYear: [{required: true, message: '请输入编制年度', trigger: 'change'}],
    comQuarter: [
      {required: true, message: '请输入编制季度', trigger: 'change'},
    ],
    coreTechnology: [
      {required: true, message: '请输入编制季度', trigger: 'change'},
      {min: 200, message: '联系电话不超过200个字符', trigger: 'change'},
    ],
    schedule: [
      {required: true, message: '请输入编制季度', trigger: 'change'},
      {min: 200, message: '联系电话不超过200个字符', trigger: 'change'},
    ],
    manual: [
      {required: true, message: '请输入人工安排', trigger: 'change'},
      {min: 200, message: '联系电话不超过200个字符', trigger: 'change'},
    ],
    equipment: [
      {required: true, message: '请输入设备安排', trigger: 'change'},
      {min: 200, message: '联系电话不超过200个字符', trigger: 'change'},
    ],
    cost: [
      {required: true, message: '请输入费用安排', trigger: 'change'},
      {min: 200, message: '联系电话不超过200个字符', trigger: 'change'},
    ],
    debug: [
      {required: true, message: '请输入调试安排', trigger: 'change'},
      {min: 200, message: '联系电话不超过200个字符', trigger: 'change'},
    ],
    other: [
      {required: true, message: '请输入其他安排', trigger: 'change'},
      {min: 200, message: '联系电话不超过200个字符', trigger: 'change'},
    ],
  };
  isUpdata = true;
  files = [{}];

  //是否可以点击下载文件按钮
  set() {
    if (this.files !== null) {
      this.isUpdata = false;
    } else {
      this.isUpdata = true;
    }
  }

  //下载文件按钮
  downFiles() {
    this.$API
        .apiDownloadFile()
        .then((res) => {
          this.loadingBtn = 0;
          this.$message({
            type: 'success',
            message: '下载成功!',
          });
        })
        .catch(() => {
          this.loadingBtn = 0;
        });
  }

  // 设置空数据
  getBaseInfo() {
    return {
      orgNumber: this.$store.getters.currentContract?.contractId,
      createUser: this.$store.getters.userInfo?.userName,
      createTime: new Date(),

      orgName: '',
      companyName: '',
      leader: '',
      station: '',
      startYear: '',
      endYear: '',
      secretLevelList: ['绝密', '普通'],
      secretLevel: '',
      conStatusList: ['已审批', '已保存'],
      conStatus: '',
      remark: '',
      //textarea
      coreTechnology: '',
      schedule: '',
      manual: '',
      equipment: '',
      cost: '',
      debug: '',
      other: '',
    };
  }

  activated() {
    if (Object.keys(this.$route.params).length > 0) {
      if (this.$route.params.initData) {
        this.initData();
      }
    }
  }

  // 初始化新建数据
  initData() {
    this.$refs['doForm'].resetFields();
    this.baseInfo = Object.assign(this.baseInfo, this.getBaseInfo());
  }

  // 保存按钮
  saveBtn() {
    const _self = this;
    let formArr = ['doForm'];
    let resultArr = [];
    formArr.forEach((item) => {
      //根据表单的ref校验
      resultArr.push(checkForm(_self, item));
    });
    Promise.all(resultArr).then(() => {
      this.$confirm('确定保存当前表单?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning',
      }).then(() => {
        // 调接口
        let params = Object.assign(
            {
              creatorConId: this.$store.getters.currentContract.contractId,
              creatorConName: this.$store.getters.currentContract.contractName,
              menuCode: this.MENU_CODE_LIST.contractList,
            },
            this.baseInfo
        );
        this.loadingBtn = 1;
        this.$API
            .apiAddDisclosure(params)
            .then((res) => {
              this.loadingBtn = 0;
              this.$message({
                type: 'success',
                message: '新增成功!',
              });
              this.$store.commit('DELETE_TAB', this.$route.path);
              this.$router.push({name: 'contractList',params:{refresh:true}});
            })
            .catch(() => {
              this.loadingBtn = 0;
            });
      });
    });
  }

  // 自定义上传
  handleUpload(file) {
    var fd = new FormData();
    fd.append('file', file.file);
    this.$API
        .apiUploadFile(fd, (progressEvent) => {
          this.onUploadProgress(progressEvent, file.file.uid);
        })
        .then((res) => {
          this.fileList.some((item, index) => {
            if (item.uid == file.file.uid) {
              this.fileList.splice(index, 1, res.data);
              return true;
            }
          });
        })
        .catch((err) => {
          this.fileList.some((item, index) => {
            if (item.uid == file.file.uid) {
              this.fileList.splice(index, 1);
              return true;
            }
          });
        });
  }

  // 返回按钮
  backBtn() {
    this.$confirm('未保存的数据将丢失，是否返回?', '提示', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning',
    }).then(() => {
      this.$store.commit('DELETE_TAB', this.$route.path);
      this.$router.push({name: 'contractList'});
    });
  }
}
</script>

<style lang="scss" scoped>
@import '@/assets/scss/element-variables.scss';

/deep/ .card-global {
  height: calc(100vh - 190px);
}

.el-form {
  /deep/ .el-form-item.large {
    width: 100%;
  }

  .el-form-item--mini {
    width: 33% !important;
  }
}
</style>
