<!--
* @description
* @fileName reportList.vue
* @author Miaoxy
* @date 2021/11/24 21:04:38
!-->
<template>
  <div class="wrapper">
    <!-- 按钮组版块 -->
...
import {apiDeleteReport, apiDownloadReportTemplate} from "../../../api/modules/implementationManagement/report";
...
export default class extends tableMixin {
  ...
  // 新建
  createBtn() {
    this.$router.push({ name: 'reportNew', params: { initData: true } })
  }
  // 编辑
  editBtn() {
    ...
    this.$confirm('您确认删除已选择的单据么?', '提示', {
      ...
      .then((e) => {
        const params = {
          ...
          menuCode: this.MENU_CODE_LIST.reportList,
          idList: this.idList,
        }
        this.loadingBtn = loadingBtnIndex
        this.$API
          .apiDeleteReport(params)
          .then((res) => {
            this.loadingBtn = 0
            this.$message({
              type: 'success',
  ...
  exportExcelBtn() {
    ...
    this.EXPORT_FILE(this.selected, 'excel', {
      url: '/rdexpense/progressreport/exportExcel',
      data,
    })
  }
  // 导出pdf
  exportPdfBtn() {
    ...
    this.EXPORT_FILE(this.selected, 'pdf', {
      url: '/rdexpense/progressreport/exportPDF',
      data,
    })
  }
  // 打印
  printBtn() {
    ...
    this.EXPORT_FILE(this.selected, 'print', {
      url: '/rdexpense/progressreport/exportPrint',
      data,
    })
  }
  // 刷新
