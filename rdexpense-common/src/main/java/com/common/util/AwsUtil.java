...
//            urlRequest.setExpiration(expiration);
     ...
     * 批量下载文件（二进制流返回）
     */
    public static void downloadFile(HttpServletResponse resp, List<PageData> list,String zipName)  {
        AmazonS3 s3 = createConnection();
        try {
            ...
            for (PageData data:list){
                S3Object o = s3.getObject(ConstantValUtil.BUCKET_PRIVATE, data.getString("file_path"));
                S3ObjectInputStream s3is = o.getObjectContent();
                byte[] read_buf = new byte[1024];
                int read_len = 0;
                while ((read_len = s3is.read(read_buf)) > 0) {
                    outputStreams.write(read_buf, 0, read_len);
                }
                ByteArrayInputStream inputStream = new ByteArrayInputStream(outputStreams.toByteArray());
                DownloadUtil.zipFile(data.getString("file_name"), inputStream, zos);
                inputStream.close();
                outputStreams.reset();
                s3is.close();
                o.close();
     ...
     * 获取文件（Base64编码返回）
     */
    public static String downloadBase64(String object_keys, String bucketName) {
        AmazonS3 s3 = createConnection();
        String result = "";
        try {
            S3Object o = s3.getObject(bucketName, object_keys);
            S3ObjectInputStream s3is = o.getObjectContent();
            result = Base64.encode(s3is);
            s3is.close();
        } catch (AmazonServiceException e) {
            e.getErrorMessage();
        } catch (FileNotFoundException e) {
