...
public class PermissionController extends BaseController {
    ...
    @GetMapping(value = "/queryDeptTreeAll")
    public ResponseEntity<List<DepartmentTreeDTO>> queryDeptTreeAll() {
        PageData pd = this.getParams();
        try {
            List<PageData> list = permissionService.queryDeptTreeAll(pd);
            return ResponseEntity.success(PropertyUtil.covertListModel(list, DepartmentTreeDTO.class), ConstantMsgUtil.INFO_QUERY_SUCCESS.desc());
        } catch (Exception e) {
            logger.error("查询部门职务树失败,request=[{}]", pd);
            return ResponseEntity.failure(ConstantMsgUtil.ERR_QUERY_FAIL.val(), e.getMessage());
    ...
    @ApiOperation(value = "查询工程树", notes = "查询工程树")
    public ResponseEntity getProjectTree() {
        return permissionService.getProjectTree();
    }
