...
public class DisclosurePaperController extends BaseController {
    ...
    public ResponseEntity addDisclosurePaper(DisclosurePaperAddDTO disclosurePaperAddDTO) {
        ...
        CheckParameter.stringIsNull(pd.getString("preparedName"), "交底书编制人");
        CheckParameter.stringIsNull(pd.getString("preparedDate"), "编制日期");
        CheckParameter.stringIsNull(pd.getString("preparedYear"), "编制年度");
        CheckParameter.stringIsNull(pd.getString("preparedQuarter"), "编制季度");
        CheckParameter.stringIsNull(pd.getString("keyTechnology"), "核心技术");
        CheckParameter.stringIsNull(pd.getString("workPlan"), "工作安排");
        CheckParameter.stringIsNull(pd.getString("artificialPlan"), "人工安排");
        CheckParameter.stringIsNull(pd.getString("equipmentPlan"), "设备安排");
        CheckParameter.stringIsNull(pd.getString("costPlan"), "费用安排");
        CheckParameter.stringIsNull(pd.getString("testPlan"), "测试安排");
        CheckParameter.stringIsNull(pd.getString("otherPlan"), "其他安排");
    ...
    public ResponseEntity<DisclosurePaperDetailDTO> getDisclosurePaperDetailBybusinessid() {
        PageData pd = this.getParams();
        CheckParameter.stringLengthAndEmpty(pd.getString("businessId"), "业务ID", 64);
        try {
            PageData pageData = disclosurePaperService.getDisclosurePaperDetailBybusinessId(pd);
            return PropertyUtil.pushData(pageData, DisclosurePaperDetailDTO.class, ConstantMsgUtil.INFO_QUERY_SUCCESS.desc());
        } catch (MyException e) {
            logger.error("查询项目交底书详情,request=[{}]", pd);
            return ResponseEntity.failure(ERR_QUERY_FAIL.val(), ERR_QUERY_FAIL.desc());
    ...
    @ApiOperation(value = "导入经济交底书模版内容")
    @PostMapping("/uploadSurvey")
    public ResponseEntity<DisclosurePaperSurveyDto> uploadSurvey(UploadTemplateFileDto dto) throws Exception{
        PageData pd = FileParamsUtil.checkParams(dto);
        ResponseEntity result = null;
        try {
            PageData pageData = disclosurePaperService.uploadSurvey(dto.getFile(),pd);
 //           result = ResponseEntity.success(pd, INFO_IMPORT_SUCCESS.desc());
            result = PropertyUtil.pushData(pageData, DisclosurePaperSurveyDto.class, ConstantMsgUtil.INFO_UPLOAD_SUCCESS.desc());
            return result;
        } catch (MyException e) {
            logger.error("导入经济交底书失败,request=[{}]", pd);
    ...
    public ResponseEntity export(IdListDto idListDto) {
        ...
        try {
            ...
            //生成excel
            disclosurePaperService.export(pd, zos, bos,serialNumber);
            zos.flush();
            zos.close();
            //写入返回response
            response.reset();
    ...
    public Object exportPrint(IdListDto idListDto) {
        ...
        ResponseEntity result = null;
        try {
            result = disclosurePaperService.export2(pd,httpServletResponse);
            return result;
        } catch (Exception e) {
            result = ResponseEntity.failure(ERR_SAVE_FAIL.val(), e.getMessage());
            throw new MyException(ERR_EXPORT_FAIL.desc(), e);
