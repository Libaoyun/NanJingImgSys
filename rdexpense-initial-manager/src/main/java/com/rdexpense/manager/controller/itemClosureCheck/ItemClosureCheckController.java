...
public class ItemClosureCheckController extends BaseController {
    ...
    public ResponseEntity<List<ResearchUserDto>> queryApplyUserList() {
        PageData pd = this.getParams();
        ResponseEntity result = null;
        try {
            List<PageData> list = itemClosureCheckService.queryApplyUserList(pd);

            result = ResponseEntity.success(PropertyUtil.covertListModel(list, ResearchUserDto.class), ConstantMsgUtil.INFO_QUERY_SUCCESS.desc());
            return result;
        } catch (Exception e) {
