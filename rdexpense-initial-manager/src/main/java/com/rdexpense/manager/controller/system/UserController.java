...
//        CheckParameter.checkDefaultParams(pd);
    ...
    private void checkParams(PageData pd) {
        ...
        String regex = "^(?![0-9]+$)(?![a-zA-Z]+$)[0-9A-Za-z]{10,15}$";
        if (!password.matches(regex)) {
            throw new MyException("密码必须包含数字和字母，且位数为10-15位");
        }
    ...
    private void checkParam(PageData pd) {
        ...
        if(!CollectionUtils.isEmpty(departmentList)){
            for(PageData department : departmentList){
                CheckParameter.stringLengthAndEmpty(department.getString("departmentName"), "部门", 1024);
                CheckParameter.stringLengthAndEmpty(department.getString("departmentCode"), "部门编码", 1024);
