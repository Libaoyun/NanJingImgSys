...
import com.rdexpense.manager.dto.base.FlowDoneQueryDto;
...
public class FlowController extends BaseController {
    ...
    public ResponseEntity<PageInfo<ApproveDoneListDTO>> queryWaitDone(FlowDoneQueryDto flowDoneQueryDto) {
        PageData pd = this.getParams();
        try {
    ...
    @ApiOperation(value = "获取代办列表中每条数据审批权限")
    @PostMapping(value = "/queryTodoAuthorization")
    @ApiImplicitParams(value = {
            @ApiImplicitParam(name = "userCode", value = "用户编码", required = true, dataType = "String"),
            @ApiImplicitParam(name = "menuCode", value = "菜单编码", required = true, dataType = "String"),
            @ApiImplicitParam(name = "creatorOrgId", value = "编制单位编码", required = true, dataType = "String")})
    public ResponseEntity queryTodoAuthorization() {
        PageData pd = this.getParams();
        CheckParameter.stringLengthAndEmpty(pd.getString("userCode"), "用户编码",128);
        CheckParameter.stringLengthAndEmpty(pd.getString("menuCode"), "菜单编码",128);
        CheckParameter.stringLengthAndEmpty(pd.getString("creatorOrgId"), "编制单位编码",256);
        try {
            PageData pageData = flowService.queryTodoAuthorization(pd);
            return ResponseEntity.success(pageData);
        } catch (Exception e) {
            logger.error("获取代办列表中每条数据审批权限失败,request=[{}]", pd);
            return ResponseEntity.failure(ConstantMsgUtil.ERR_QUERY_FAIL.val(), e.getMessage());
        }
    }
    ...
    public ResponseEntity<PageInfo<ApproveDoneListDTO>> queryIsDone(FlowDoneQueryDto flowDoneQueryDto) {
        PageData pd = this.getParams();
        try {
