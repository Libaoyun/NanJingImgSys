...
public class ProjectApplyServiceImpl implements ProjectApplyService {
    ...
    public List queryList(PageData pd) {
        ...
        List<String> bureauLevelList = JSONArray.parseArray(pd.getString("bureauLevel"), String.class);
        pd.put("bureauLevelList", bureauLevelList);
    ...
    public void addApply(PageData pd) {
        ...
        //进行项目唯一性校验
        List<PageData> list = (List<PageData>) dao.findForList("ProjectApplyMapper.queryByProjectName",pd);
        if(!CollectionUtils.isEmpty(list)){
            throw new MyException("该项目名称已存在");
        }
        ...
        //bureauLevel 是否为局级 0：否1：是
        if(pd.getString("bureauLevel").equals("1")){
            dicList.add("1024");
        }else {
            dicList.add("1023");
        }
        ...
        List<PageData> dicTypeList = (List<PageData>) dao.findForList("DictionaryTypeMapper.selectListByParentIds", dicList);
  ...
  //                  throw new MyException(expenseaccount + "填写错误，请调整重新上传");
   ...
   //         dao.batchInsert("ProjectApplyMapper.batchInsertBudgetMonth", monthList);
        ...
        for (PageData detailData : monthList) {
            if(!CollectionUtils.isEmpty(yearSet)){
                ...
                for(String year : yearSet){
                    ...
                    data.put("expenseaccountcode", detailData.getString("expenseaccountcode"));
    ...
    public PageData getApplyDetail(PageData pd) {
        ...
        //查询周期变更
//        PageData changeData = (PageData) dao.findForObject("ProjectApplyMapper.queryChangeDate",pd);
//        if(changeData != null){
//            request.put("startYear",changeData.getString("startYear"));
//            request.put("endYear",changeData.getString("endYear"));
//        }
        ...
        if (request != null) {
            ...
            // 5、研究人员（变更）
            pd.put("projectId",pd.getString("id"));
            ...
            //先取变更的预算，如果没有则取初始预算
            List<PageData> budgetChangeList = (List<PageData>) dao.findForList("ProjectApplyMapper.queryChangeBudget", request);
            if(CollectionUtils.isEmpty(budgetChangeList)){
                ...
                request.put("budgetList", budgetChangeList);
            }


            // 8、经费预算（每月预算) 年度预算（按月填报)
            List<PageData> monthListNew = (List<PageData>) dao.findForList("ProjectApplyMapper.queryMonthList", request);

            if(!CollectionUtils.isEmpty(monthListNew)){
                request.put("monthListNew", monthListNew);
            }
            // 8、经费预算（每月预算) 年度预算（按月填报)
            ...
            if(!CollectionUtils.isEmpty(monthList)){
                ...
                request.put("monthDetailList", monthDetailList);
...
//        String businessId = (String)dao.findForObject("");
    ...
    public void uploadAll(MultipartFile file, PageData pd) {
        ...
        try {
            ...
            throw new MyException("请上传正确的项目立项模板");
        }
 ...
 //           dao.batchInsert("ProjectApplyMapper.batchInsertBudget", budgetList);
     ...
     */
    @Override
    public PageData uploadMain(MultipartFile file, PageData pd){
     ...
     */
    @Override
    public PageData uploadSurvey(MultipartFile file, PageData pd){
        XSSFWorkbook wb = null;
        try {
            wb = getWorkbook(file);
        } catch (Exception e) {
     ...
     */
    @Override
    public List<PageData> uploadProgress(MultipartFile file, PageData pd){
        XSSFWorkbook wb = null;
        try {
            wb = getWorkbook(file);
        } catch (Exception e) {
     ...
     */
    @Override
    public List<PageData> uploadUnit(MultipartFile file, PageData pd){
        XSSFWorkbook wb = null;
        try {
            wb = getWorkbook(file);
        } catch (Exception e) {
     ...
     */
    @Override
    public List<PageData> uploadUser(MultipartFile file, PageData pd){
        XSSFWorkbook wb = null;
        try {
            wb = getWorkbook(file);
        } catch (Exception e) {
     ...
     */
    @Override
    public List<PageData> uploadBudget(MultipartFile file, PageData pd){
        XSSFWorkbook wb = null;
        try {
            wb = getWorkbook(file);
        } catch (Exception e) {
     ...
     */
    @Override
    public List<PageData> uploadMonth(MultipartFile file, PageData pd){
        XSSFWorkbook wb = null;
        try {
            wb = getWorkbook(file);
        } catch (Exception e) {
    ...
    private List<PageData> packageMonthList(List<PageData> monthList,List<PageData> monthDetailList){
        ...
        if(!CollectionUtils.isEmpty(monthDetailList)){
            for(PageData data : monthList){
                data.put("expensebudget",data.getString("expensebudget"));
                data.put("sourcebudget",data.getString("sourcebudget"));

                String expenseaccountcode = data.getString("expenseaccountcode");
                ...
                for(Map.Entry<String,List<PageData>> entry :map.entrySet()){
                    ...
                    for (PageData detailData : valueList) {
                        if (detailData.getString("expenseaccountcode").equals(expenseaccountcode)) {
                            for(int i=0;i<12;i++){
                                String keyParam = "month"+key+""+(i+1);
                                data.put(keyParam,detailData.getString(arr[i]));
     ...
     */
    @Override
    public List<PageData> uploadAppropriation(MultipartFile file, PageData pd){
        XSSFWorkbook wb = null;
        try {
            wb = getWorkbook(file);
        } catch (Exception e) {
    ...
    private PageData getMainData(XSSFWorkbook wb,PageData pd) {
        ...
        //14、是否鉴定
        cell = row.getCell(10);
        String identify = ReadExcelUtil.readCellStr(cell, 8, "是否鉴定", false, 256);
        ...
        row = sheet.getRow(11);
        cell = row.getCell(1);
        String reviewComments = ReadExcelUtil.readCellStr(cell, 12, "申报单位审查意见", true,50000);
    ...
    private List<PageData> getProgressPlan(XSSFWorkbook wb,PageData pd) {
        ...
        for (int i = 2; i <= end; i++) {
            ...
            int rowNumber = i + 1;
            //解析第一个单元格 年度
            String years = ReadExcelUtil.readCellYear(cell0, rowNumber, "进度计划年度", false);
...
//        checkHead(sheet,head);
        ...
        for (int i = 2; i <= end; i++) {
            ...
            int rowNumber = i + 1;
            //解析第1个单元格 姓名
            String userName = ReadExcelUtil.readCellFormat(cell1, rowNumber, "姓名", false, 256);
            ...
            //解析第3个单元格 年龄
            String age = ReadExcelUtil.readCellFormat(cell3, rowNumber, "年龄", false, 256);
            data.put("age",age);

            //解析第4个单元格 性别
            String gender = ReadExcelUtil.readCellFormat(cell4, rowNumber, "性别", false, 256);
            data.put("gender",gender);

            //解析第5个单元格 学历
            String education = ReadExcelUtil.readCellFormat(cell5, rowNumber, "学历", false, 256);
            data.put("education",education);

            //解析第6个单元格 所属部门
            String belongDepartment = ReadExcelUtil.readCellFormat(cell6, rowNumber, "所属部门", false, 256);
            data.put("belongDepartment",belongDepartment);

            //解析第7个单元格 职务职称
            String belongPost = ReadExcelUtil.readCellFormat(cell7, rowNumber, "职务职称", false, 256);
            data.put("belongPost",belongPost);

            //解析第8个单元格 所学专业
            String majorStudied = ReadExcelUtil.readCellFormat(cell8, rowNumber, "所学专业", false, 256);
            data.put("majorStudied",majorStudied);

            //解析第9个单元格 现从事专业
            String majorWorked = ReadExcelUtil.readCellFormat(cell9, rowNumber, "现从事专业", false, 256);
            data.put("majorWorked",majorWorked);

            //解析第10个单元格 所在单位
            String belongUnit = ReadExcelUtil.readCellFormat(cell10, rowNumber, "所在单位", false, 256);
            data.put("belongUnit",belongUnit);

            //解析第11个单元格 研究任务及分工
            String taskDivision = ReadExcelUtil.readCellFormat(cell11, rowNumber, "研究任务及分工", false, 256);
            data.put("taskDivision",taskDivision);

            //解析第12个单元格 全时率
            String workRate = ReadExcelUtil.readCellFormat(cell12, rowNumber, "全时率", false, 256);
            data.put("workRate",workRate);

            //解析第13个单元格 联系电话
            String telephone = ReadExcelUtil.readCellFormat(cell13, rowNumber, "联系电话", false, 256);
 ...
 //       checkHead(sheet,head);
        ...
        for (int i = 2; i <= end; i++) {
            ...
            int rowNumber = i + 1;
            //解析第一个单元格 年度
            String years = ReadExcelUtil.readCellYear(cell0, rowNumber, "拨款计划年度", false);
            data.put("years",years);

            //解析第二个单元格 计划（万元）
            String planAmount = ReadExcelUtil.readCellDecimal(cell1, rowNumber, "计划（万元）", false, 20, 2);
    ...
    private List<PageData> packageBudget(XSSFSheet sheet,List<PageData> list,PageData pd){
        ...
        if(pd.getString("bureauLevel").equals("1")){
            //解析第17个单元格 九、其他费用
            setBudgetValue(sheet,21,-1,-1,2,3,list,pd,map);
        }
    ...
    //校验预算中小项的和是否等于大项
    private void checkAmount(List<PageData> list,PageData pd){
        if(!CollectionUtils.isEmpty(list)){
            //局级模板
            if(pd.getString("bureauLevel").equals("1")){
                //一、人员费
                ...
                //人员费小项合计
                BigDecimal userAmountSum = new BigDecimal(0);

                //三、材料及加工试验费
                BigDecimal matAmount = new BigDecimal(0);
                //材料费小项合计
                BigDecimal matAmountSum = new BigDecimal(0);

                //四、相关业务费
                BigDecimal busAmount = new BigDecimal(0);
                //相关业务费小项合计
                BigDecimal busAmountSum = new BigDecimal(0);




                for(PageData data : list){
                    String code = data.getString("expenseaccountcode");
                    String value = data.getString("expensebudget");
                    if(StringUtils.isBlank(value)){
                        continue;
                    }

                    if(code.equals("102411")){
                        userAmount = new BigDecimal(value);
                    }else if(code.equals("102412")||code.equals("102413")){
                        userAmountSum = userAmountSum.add(new BigDecimal(value));
                    }else if(code.equals("102415")){
                        matAmount = new BigDecimal(value);
                    }else if(code.equals("102416")||code.equals("102417")){
                        matAmountSum = matAmountSum.add(new BigDecimal(value));
                    }else if(code.equals("102418")){
                        busAmount = new BigDecimal(value);
                    }else if(code.equals("102419")||code.equals("102420")||code.equals("102421")||code.equals("102422")){
                        busAmountSum = busAmountSum.add(new BigDecimal(value));
                    }

                }

                if(userAmount.compareTo(userAmountSum) != 0){
                    throw new MyException("模板中人员费和下面小项合计值不相等");
                }

                if(matAmount.compareTo(matAmountSum) != 0){
                    throw new MyException("模板中材料及加工试验费和下面小项合计值不相等");
                }

                if(busAmount.compareTo(busAmountSum) != 0){
                    throw new MyException("模板中相关业务费和下面小项合计值不相等");
                }

            }else if(pd.getString("bureauLevel").equals("0")){//其他模板
                //九、其他费用
                ...
                for(PageData data : list){
                    ...
                    if(code.equals("102326")){
                        otherAmount = new BigDecimal(value);
                    }else if(code.equals("102319")||code.equals("102320")||code.equals("102321")||code.equals("102322")||code.equals("102323")){
                        sumAmount = sumAmount.add(new BigDecimal(value));
                    }

                }

                if(otherAmount.compareTo(sumAmount) != 0){
                    throw new MyException("模板中其他费用和下面小项合计值不相等");
                }
    ...
    List<PageData> getBudgetMonthDetail(XSSFWorkbook wb,PageData pd,List<PageData> monthList){
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (value.equals("1月")) {//表示填数据了
                ...
                int cellNum = 4 + i * 12;
                for (int y = 4; y <= index; y++) {
                    PageData data = new PageData();
                    data.put("businessId", pd.getString("businessId"));
                    data.put("years", yearValue);
                    data.put("expenseaccountcode",monthList.get(y-4).getString("expenseaccountcode"));
    ...
    public void preview(PageData pd,HttpServletResponse response) throws Exception {
        ...
        try {
            is = this.getClass().getResourceAsStream("/template/研究与开发项目合同范本新.docx");
        ...
        try {
            ...
            //查询所属公司的名称及信用代码
            String companyName2  = "";
//            PageData company = (PageData) dao.findForObject("ExpensesBillSummaryMapper.queryCompany",pd);
...
//                    "\n\n开户银行：\n"+"\n\n帐户名称：\n"+"\n\n帐   号：";
            ...
            //1、主信息
            if (!pd.getString("createTime").isEmpty()){
                String year = pd.getString("createTime").substring(0,4);
...
//            }
            ...
            exportWordHelper.setCellStyle(doc,1,0,0);
            exportWordHelper.setCellStyle(doc,1,1,0);
            ...
            exportWordHelper.setCellStyle(doc,2,0,0);
            exportWordHelper.setCellStyle(doc,2,1,0);
            ...
            if(!CollectionUtils.isEmpty(researchUserList)){
                ...
                for(int i=0;i<researchUserList.size();i++){
                    PageData user = researchUserList.get(i);
                    List<String> userDetailList = new ArrayList<>();
//                    int index = i + 1;
//                    userDetailList.add(String.valueOf(index));
            ...
            exportWordHelper.setCellStyle(doc,3,0,0);
            exportWordHelper.setCellStyle(doc,3,1,0);
            exportWordHelper.setCellStyle(doc,3,2,0);
            exportWordHelper.setCellStyle(doc,3,3,0);
            exportWordHelper.setCellStyle(doc,3,4,0);
            exportWordHelper.setCellStyle(doc,3,5,0);
...
//            exportWordHelper.exportWordDisorder1(companyName3, doc, 6,1,0);
            ...
            xwpfTUtil.close(is);
            throw new MyException("研究与开发项目合同导出异常",e);
...
//        InputStream is = null;
        ...
        InputStream is = null;
        try {
            is = this.getClass().getResourceAsStream("/template/研发项目立项申请信息1.docx");
            doc = new XWPFDocument(is);
    ...
    private File packageWord(String businessId,XWPFDocument doc,InputStream is,String fileName,File file){
        ...
        List<PageData> researchUser = new LinkedList<>();
        List<PageData> researchUserChange = (List<PageData>)data.get("researchUserChange");
        List<PageData> researchUserOriginal = (List<PageData>)data.get("researchUser");
        if(!CollectionUtils.isEmpty(researchUserChange)){
            researchUser.addAll(researchUserChange);
        }else if(!CollectionUtils.isEmpty(researchUserOriginal)){
            researchUser.addAll(researchUserOriginal);
        }
        ...
        try {
            ...
            Map<String, Object> params = new HashMap<>();
            //1、主信息
            String year = data.getString("createTime").substring(0,4);
            params.put("(year)", year);
...
//            }
            ...
            exportWordHelper.setCellStyle(doc,3,0,0);
            exportWordHelper.setCellStyle(doc,3,0,1);
            exportWordHelper.setCellStyle(doc,3,1,1);
            exportWordHelper.exportWordDisorder(progressList, doc, 3);
            ...
            exportWordHelper.exportWordDisorder(unitList, doc, 4);
            ...
            if(!CollectionUtils.isEmpty(researchUser)){
                PageData leader = researchUser.get(0);
                researchUser.add(0,leader);
                PageData data1 = new PageData();
                data1.put("userName","主要研究人员");
                researchUser.add(1,data1);
          ...
          //          int number = i+1;
                    ...
                    if(i==0){
                        userDetailList.add("1");
                    }else if(i==1){
                        userDetailList.add(user.getString("userName"));
                    }else{
                        userDetailList.add(String.valueOf(i));
                    }
            ...
            exportWordHelper.exportWordDisorder(userList, doc, 5);
            exportWordHelper.mergeCellsVertically(doc,5,0,4,2);
            ...
            exportWordHelper.exportWordDisorder(budgetList1, doc, 6);
            ...
            exportWordHelper.exportWordDisorder(appList, doc, 7);
            ...
            exportWordHelper.exportWordDisorder(approveList, doc, 8);
