...
public class FlowServiceImpl implements FlowService {
    ...
    public PageData startFlow(PageData pd) {
        ...
        if (flowData != null) {
            ...
            if (flowContent != null) {
                ...
                if (startData != null) {
                    String assignUserId = pd.getString("assignUserId");
                    getNextApproveNode(assignUserId,lineList,nodeList,startData.getString("id"),nextApproveNodeList);
                }else {
                ...
                nextApproveNode.put("menuCode",pd.getString("menuCode"));
                nextApproveNode.put("creatorOrgId",pd.getString("creatorOrgId"));
                //查询下一个节点的审批人员
                List<PageData> approvalUserList = getApprovalUserList(nextApproveNode);
                logger.info("查询下一个节点的审批人员："+approvalUserList);

                logger.info("指定下一个节点的审批人员传参："+pd.getString("assignUserId"));
                //todo 判断是否指定下一个审批人员
                List<PageData> assignApprovalUserList = new ArrayList<>();
                if (pd.getString("assignUserId") != null && !pd.getString("assignUserId").isEmpty()){
                    for (PageData pageData : approvalUserList) {
                        if (pageData.getString("userCode").equals(pd.getString("assignUserId"))){
                            pageData.put("userCode",pd.getString("assignUserId"));
                            pageData.put("handleUserId",pd.getString("assignUserId"));
                            assignApprovalUserList.add(pageData);
                        }
                    }
                }

                logger.info("指定下一个节点的审批人员节点信息："+assignApprovalUserList);
                ...
                //封装下一个审批人
                PageData nextUser = new PageData();
                if (!CollectionUtils.isEmpty(assignApprovalUserList)){
                    nextUser = transforNextUser(assignApprovalUserList);

                } else {
                    nextUser = transforNextUser(approvalUserList);
                }
                ...
                if(processInstId != null){
                    if (!CollectionUtils.isEmpty(assignApprovalUserList)){
                        insertApproveDone(pd,startData,nextApproveNode,nextUser,assignApprovalUserList);
                    } else {
    ...
    public PageData approveFlow(PageData pd) {
        ...
        PageData nextApproveNode = null;
                getNextApproveNode(assignUserId,lineList, nodeList, currentApproveNodeId,nextApproveNodeList);
        if(CollectionUtils.isEmpty(nextApproveNodeList)){
...
//                }
                ...
                currentApproveNode.put("creatorOrgId",pd.getString("creatorOrgId"));

                approvalUserList = getApprovalUserList(currentApproveNode);
            }
...
//        PageData nextUser = transforNextUser(approvalUserList);
        ...
        insertApproveSchedule(pd,currentApproveNode,nextApproveNode,nextUser,
                pd.getString("approveComment"),ConstantValUtil.APPROVAL_RESULT[1],ConstantValUtil.APPROVAL_RESULT_NAME[1]);
        ...
        if(result == 1 && bool == true){
            if (!CollectionUtils.isEmpty(assignApprovalUserList)){
                insertApproveDone(pd,currentApproveNode,nextApproveNode,nextUser,assignApprovalUserList);
            } else {
    ...
    public PageData backPreviousNode(PageData pd) {
        ...
        if(type.equals("start")){
            ...
            insertApproveSchedule(pd,currentApproveNode,new PageData(),new PageData(),
                    pd.getString("approveComment"),ConstantValUtil.APPROVAL_RESULT[3],ConstantValUtil.APPROVAL_RESULT_NAME[3]);
...
//            pd.put("processInstId",null);
            ...
            //找到当前节点
            PageData currentApproveNode = getCurrentApproveNode( nodeList, currentApproveNodeId);
            currentApproveNode.put("menuCode",pd.getString("menuCode"));
            currentApproveNode.put("creatorOrgId",pd.getString("creatorOrgId"));
            //找到当前节点的审批人
            ...
            previousApproveNode.put("creatorOrgId",pd.getString("creatorOrgId"));

            //找到上一个节点的审批人
            List<PageData> previousUserList = getApprovalUserList(previousApproveNode);
            //todo 判断是否指定了下一个审批人员
            List<PageData> assignApprovalUserList = new ArrayList<>();
            if (previousApproveNode.getString("assignUserId") != null && !previousApproveNode.getString("assignUserId").isEmpty()){
                for (PageData pageData : previousUserList) {
                    if (pageData.getString("userCode").equals(previousApproveNode.getString("assignUserId"))){
                        assignApprovalUserList.add(pageData);
                    }
                }
            }
            ...
            //封装上一个节点的审批人
            PageData previousUser = new PageData();
            if (!CollectionUtils.isEmpty(assignApprovalUserList)){
                previousUser = transforNextUser(assignApprovalUserList);
            } else {
                previousUser = transforNextUser(previousUserList);
            }
            ...
            insertApproveSchedule(pd,currentApproveNode,previousApproveNode,currentUser,
                    pd.getString("approveComment"),ConstantValUtil.APPROVAL_RESULT[2],ConstantValUtil.APPROVAL_RESULT_NAME[2]);
            ...
            //7、插入新的待办信息
            if (previousApproveNode.getString("assignUserId") != null && !previousApproveNode.getString("assignUserId").isEmpty()){
                insertApproveDone(pd,currentApproveNode,previousApproveNode,previousUser,assignApprovalUserList);
            } else {
                insertApproveDone(pd,currentApproveNode,previousApproveNode,previousUser,previousUserList);
            }


            pd.put("processStatus",ConstantValUtil.APPROVAL_STATUS[1]);
            pd.put("nextApproveUserId",previousUser.getString("userCode"));
            pd.put("nextApproveUserName",previousUser.getString("userName"));
...
//        pd.put("processInstId",null);
    ...
    private void insertApproveDone(PageData pd,PageData currentApproveNode,PageData nextApproveNode,PageData nextUser,List<PageData> approvalUserList){
        ...
        doneData.put("creatorOrgId",pd.getString("creatorOrgId"));
     ...
     * @return
     */
    private void getNextApproveNode(String assignUserId,List<PageData> lineList,List<PageData> nodeList,String currentNodeId,List<PageData> nextApproveNodeList){
        ...
        if (StringUtils.isNotBlank(nextNode)) {
            for (PageData nodeData : nodeList) {
                ...
                if (nodeData.getString("id").equals(nextNode)) {
                    //当该节点是禁用的，则继续往下寻找
                    if(status.equals("0")){
                        getNextApproveNode(assignUserId,lineList,nodeList,nextNode,nextApproveNodeList);
                    }else{
                        //todo 判断是否有指定审批人，给审批流程添加指定审批人标识，回退时使用
                        if (assignUserId != null && StringUtils.isNotEmpty(assignUserId)){
                            nodeData.put("assignUserId",assignUserId);
                        }
    ...
    private List<PageData> getApprovalUserList(PageData pd) {
        ...
        if (!CollectionUtils.isEmpty(handleRangeList)) {
            ...
            List<PageData> newUserList = new ArrayList<>();
            //todo 遍历流程配置节点的人员和职务信息
            for (PageData hRangePageData : handleRangeList) {

                PageData scopePageData = new PageData();
                scopePageData.put("scopeCode",pd.getString("creatorOrgId"));
                scopePageData.put("menuCode", pd.getString("menuCode"));
                scopePageData.put("userCode", hRangePageData.getString("handleUserId"));
                scopePageData.put("userName", hRangePageData.getString("handleUserName"));


                //todo 流程配置节点是人员
                if (hRangePageData.getString("handleType").equals("user")){

                    //todo 查询人员在系统中是否存在 queryUser
                    List<PageData> userList = (List<PageData>) baseDao.findForList("FlowMapper.selectUserByUserId", scopePageData);
                    if (CollectionUtils.isEmpty(userList)) {
                        throw new MyException("下个审批节点设置的审批人在系统中不存在");
                    }

                    //todo 根据人员、授权项目、菜单，查询人员是否授权 queryUserMenuById
                    List<PageData> userMenuList = (List<PageData>) baseDao.findForList("FlowMapper.selectUserMenuByUserId", scopePageData);
                    if(!CollectionUtils.isEmpty(userMenuList)){
                        hRangePageData.put("userCode", hRangePageData.getString("handleUserId"));
                        hRangePageData.put("userName", hRangePageData.getString("handleUserName"));
                        newUserList.add(hRangePageData);
                    } else {
                        //todo 人员未授权，根据人员，查询人员职务 queryPostByUserId
                        List<PageData> postList = (List<PageData>) baseDao.findForList("FlowMapper.selectPostByUserId",scopePageData);
                        if (!CollectionUtils.isEmpty(postList)){

                            scopePageData.put("valueList", postList);

                            //todo 根据职务、授权项目、菜单，查询授权项目的人员职务是否授权 queryUserMenuById
                            List<PageData> postMenuList = (List<PageData>) baseDao.findForList("FlowMapper.selectUserMenuByPostCode", scopePageData);
                            if(!CollectionUtils.isEmpty(postMenuList)){
                                hRangePageData.put("userCode", hRangePageData.getString("handleUserId"));
                                hRangePageData.put("userName", hRangePageData.getString("handleUserName"));
                                newUserList.add(hRangePageData);
                            }
                ...
                } else if (hRangePageData.getString("handleType").equals("post")){
                    //todo 职务授权，
                    //todo 根据职务、授权项目、菜单，查询是否授权
                    List<PageData> postMenuList = (List<PageData>) baseDao.findForList("FlowMapper.selectPostUserMenuByPostCode", scopePageData);
                    if(!CollectionUtils.isEmpty(postMenuList)) {
                        //todo 再查询职务下面的人 queryUserByPost
                        List<PageData> userList = (List<PageData>) baseDao.findForList("FlowMapper.selectUserByPost", postMenuList);
                        if (!CollectionUtils.isEmpty(userList)) {
                            newUserList.addAll(userList);
                        }
                    } else {
                        //todo 如果职务没有授权，再查询职务下面的人是否授权
                        //todo 再查询职务下面的人 queryUserByPost
                        List<PageData> userList = (List<PageData>) baseDao.findForList("FlowMapper.selectPostUserByPost", scopePageData);
                        if (!CollectionUtils.isEmpty(userList)) {
                            //todo 再查询职务下面的人是否授权 queryUserMenuById
                            scopePageData.put("valueList", userList);

                            List<PageData> postMenuList1 = (List<PageData>) baseDao.findForList("FlowMapper.selectUserMenuByUserCode", scopePageData);
                            if(!CollectionUtils.isEmpty(postMenuList1)){
                                for (PageData pageData : postMenuList1) {
                                    for (PageData data : userList) {
                                        if (pageData.getString("userId").equals(data.getString("userCode"))){
                                            PageData newUserPageData = new PageData();
                                            newUserPageData.put("userCode", data.getString("userCode"));
                                            newUserPageData.put("handleUserId", data.getString("userCode"));
                                            newUserPageData.put("userName", data.getString("userName"));
                                            newUserPageData.put("handleType", hRangePageData.getString("handleType"));
                                            newUserPageData.put("handleUserName", hRangePageData.getString("handleUserName"));
                                            newUserList.add(newUserPageData);
                                        }
                                    }
                                }
                            }
            ...
            if (!CollectionUtils.isEmpty(newUserList)){
                //todo 对人员进行去重
                List<PageData> userList = newUserList.stream().collect(
                        Collectors.collectingAndThen(
                                Collectors.toCollection(() -> new TreeSet<>(Comparator.comparing(e->e.getString("userCode")))), ArrayList::new)
                );
                list.addAll(userList);

            } else {
                throw new MyException("下个审批节点设置的审批人未授权此菜单权限");
            }
        } else {
            throw new MyException("下个审批节点未设置审批人或职务");
        }
        return list;
    }
    ...
    //根据审批节点，查询审批人
    private List<PageData> getApprovalUserList1(PageData pd) {
        List<PageData> list = new ArrayList<>();
        ...
        List<PageData> handleRangeList = JSONObject.parseArray(pd.getString("handleRange"),PageData.class);
        System.out.println("00000000000000==========="+handleRangeList);
        if (!CollectionUtils.isEmpty(handleRangeList)) {
            ...
            if (map != null && map.size() > 0) {
                ...
                List<PageData> menuList = new ArrayList<>();
                List<PageData> newUserList = new ArrayList<>();

                for (Map.Entry<String, List<PageData>> entry : map.entrySet()) {
                    String key = entry.getKey();
                    List<PageData> value = entry.getValue();
                    System.out.println(key+"1111111111111==========="+value);
                    if (!CollectionUtils.isEmpty(value)){
                        for (PageData pageData : value) {
                            pageData.put("menuCode",pd.getString("menuCode"));
                        }
                    }
                    logger.info("查询审批人的传参："+value.toString());
                    System.out.println("查询审批人的传参："+value.toString());
                    //查询此人是否授权此菜单的权限
                    if(!CollectionUtils.isEmpty(value)){
                        //todo 流程配置中配置的是人员
                        if(key.equals("user")){
                            ...
                            scopePageData.put("valueList",value);

                            //todo 根据人员，查询人员是否授权
                            List<PageData> userMenuList = (List<PageData>) baseDao.findForList("FlowMapper.queryUserMenuById", scopePageData);
                            if(!CollectionUtils.isEmpty(userMenuList)){
                                //todo 多人的情况下，有的人授权，有的人未授权
                                for (PageData data : userList) {
                                    boolean flag = false;
                                    //todo 过滤未授权的人
                                    for (PageData pageData : userMenuList) {
                                        if (pageData.getString("userId").equals(data.getString("userCode"))){
                                            flag = true;
                                    ...
                                    //todo 如果某人未授权，再查找此人职务是否授权
                                    if (!flag){
                                        //todo 如果人员未授权，根据审批人,查询审批人的职务
                                        List<PageData> postList = (List<PageData>) baseDao.findForList("FlowMapper.queryPostByUserId",value);
                                        if (!CollectionUtils.isEmpty(postList)){
                                            ...
                                            //todo 根据职务查询是否授权
                                            List<PageData> postMenuList = (List<PageData>) baseDao.findForList("FlowMapper.queryUserMenuById", scopePageData);
                                            if(!CollectionUtils.isEmpty(postMenuList)){
                                                //todo 根据权限职务，查询相关人员
                                                for (PageData pageData : postMenuList) {
                                                    //todo 过滤未授权的职务
                                                    if (!CollectionUtils.isEmpty(postList)){
                                                        for (PageData data1 : postList) {
                                                            if (pageData.getString("userId").equals(data1.getString("handleUserId"))){
                                                                //todo 根据授权的职务信息，获取职务下的人员
                                                                if (!CollectionUtils.isEmpty(userList)){
                                                                    for (PageData pageData1 : userList) {
                                                                        if (pageData1.getString("userCode").equals(data1.getString("userCode"))){
                                                                            newUserList.add(pageData1);
                                                                        }
                                                                    }
                                                                }
                                                            }
                                                ...
                                                menuList.addAll(postMenuList);
                                            }
                                        }
                                    }
                                }

                                menuList.addAll(userMenuList);
                            } else {
                                //todo 如果人员未授权，根据审批人,查询审批人的职务
                                List<PageData> postList = (List<PageData>) baseDao.findForList("FlowMapper.queryPostByUserId",value);
                                if (!CollectionUtils.isEmpty(postList)){
                                    ...
                                    if(!CollectionUtils.isEmpty(postMenuList)){
                                        ...
                                        for (PageData pageData : postMenuList) {
                                            //todo 过滤未授权的职务
                                            if (!CollectionUtils.isEmpty(postList)){
                                                for (PageData data : postList) {
                                                    if (pageData.getString("userId").equals(data.getString("handleUserId"))){
                                                        //todo 根据授权的职务信息，获取职务下的人员
                                                        if (!CollectionUtils.isEmpty(userList)){
                                                            for (PageData pageData1 : userList) {
                                                                if (pageData1.getString("userCode").equals(data.getString("userCode"))){
                                                                    newUserList.add(pageData1);
                                                                }
                                                            }
                                                        }
                            ...
                            //todo 流程配置中节点配置的是职务
                            scopePageData.put("valueList",value);
                            //todo 先查询职务是否授权,职务已授权
                            List<PageData> postMenuList = (List<PageData>) baseDao.findForList("FlowMapper.queryUserMenuById", scopePageData);
                            if(!CollectionUtils.isEmpty(postMenuList)) {
                                menuList.addAll(postMenuList);

                                //todo 配置多职务的情况下，过滤掉未授权的职务
                                for (PageData postValue : value) {
                                    boolean flag = false;
                                    for (PageData pageData : postMenuList) {
                                        if (postValue.getString("handleUserId").equals(pageData.getString("userId"))){
                                            flag = true;
                                        }
                                    }

                                    //todo 当职务已授权，直接获取职务下面的人
                                    if (flag){
                                        //todo 再查询职务下面的人
                                        ...
                                        if (!CollectionUtils.isEmpty(userList)) {
                                            for (PageData pageData : userList) {
                                                pageData.put("menuCode",pd.getString("menuCode"));
                                                //todo 过滤未授权的人
                                                for (PageData data : postMenuList) {
                                                    if (pageData.getString("postCode").equals(data.getString("userId"))){
                                                        newUserList.add(pageData);
                                                    }
                                                }
                                            }
                                        }
                                        ...
                                        //todo 查询职务下面的人是否授权
                                        List<PageData> postMenuList1 = (List<PageData>) baseDao.findForList("FlowMapper.queryUserMenuById", scopePageData);
                                        if(!CollectionUtils.isEmpty(postMenuList1)){
                                            menuList.addAll(postMenuList1);
                                            for (PageData pageData : postMenuList1) {
                                                //todo 过滤未授权的人
                                                if (!CollectionUtils.isEmpty(userList)){
                                                    for (PageData data : userList) {
                                                        if (pageData.getString("userId").equals(data.getString("userCode"))){
                                                            data.put("handleUserId",data.getString("userCode"));
                                                            data.put("menuCode",pageData.getString("menuCode"));
                                                            newUserList.add(data);
                                                        }
                                                    }
                                                }
     ...
     */
    @Override
    public PageData queryTodoAuthorization(PageData pd) {
        PageData returnPageData = new PageData();
        returnPageData.put("isApprove","0");

        //todo 根据人员、菜单ID、项目编码，获取审批人的审批权限
        PageData pageData = (PageData) baseDao.findForObject("FlowMapper.queryTodoAuthorizationByUserId",pd);
        if (pageData != null){
            if (pageData.getString("authority_button_code").contains("a10004")){
                returnPageData.put("isApprove","1");
            }
        }

        //todo 根据人查询所属岗位信息
        List<PageData> postList = (List<PageData>) baseDao.findForList("FlowMapper.queryTodoAuthorizationPostByUserId",pd);
        if (!CollectionUtils.isEmpty(postList)){
            //todo 根据岗位信息，获取岗位的审批权限
            pd.put("postList",postList);
            List<PageData> menuList = (List<PageData>) baseDao.findForList("FlowMapper.queryTodoAuthorizationMenuByPost",pd);
            if (!CollectionUtils.isEmpty(menuList)){
                for (PageData data : menuList) {
                    if (data.getString("authority_button_code").contains("a10004")){
                        returnPageData.put("isApprove","1");
                    }
                }
            }
     ...
     */
    @Override
    public PageData queryNextApprovalUserList(PageData pd) {
        PageData returnPageData = new PageData();
        List<PageData> userList = new ArrayList<>();
        List<PageData> approvalUserList = new ArrayList<>();
        returnPageData.put("handleStrategy","2");
        returnPageData.put("userList",userList);

        String assignUserId = "";

        //todo 判断是提交时还是审批时获取审批人员信息
        if (pd.getString("nodeType").equals("0")){
            //todo 提交时获取审批人员信息
            //todo 1、查询该功能菜单下配置的工作流
            PageData flowData = (PageData) baseDao.findForObject("FlowMapper.queryByMenuCode", pd);
            if (flowData != null) {
                //2、查询下一个审批人
                String flowContent = flowData.getString("flowContent");
                if (flowContent != null) {
                    ...
                    PageData startData = null;
                    for (PageData nodeData : nodeList) {
                        //todo type是start表示是开始节点，找到节点后，找下一个节点
                        String type = nodeData.getString("type");
                        if (type.equals("start")) {
                            startData = nodeData;
                            break;
                    ...
                    //todo 判断该节点是什么处理策略 1：一人同意即可(默认)，2：所以人必须同意，会签节点
                    if (nextApproveNode.getString("handleStrategy").equals("1")){
                        //todo 不是会签节点，需要选择审批人
                        nextApproveNode.put("menuCode",pd.getString("menuCode"));
                        nextApproveNode.put("creatorOrgId",pd.getString("creatorOrgId"));
                        //todo 查询下一个节点的审批人员
                        ...
                        returnPageData.put("handleStrategy",nextApproveNode.getString("handleStrategy"));
            ...
            //todo 6、先判断该节点是什么处理策略 1：一人同意即可(默认)，2：所以人必须同意
            String handleStrategy = waitData.getString("handleStrategy");
            if(handleStrategy.equals("1") && !nextApproveNode.getString("type").equals("end")){
                //todo 查找下一个审批人
                approvalUserList = getApprovalUserList(nextApproveNode);
                returnPageData.put("handleStrategy",nextApproveNode.getString("handleStrategy"));

            }  else if (handleStrategy.equals("2") && !nextApproveNode.getString("type").equals("end")){
                //todo 判断审批人是不是最后一个人审批，
                //todo 如果是最后一个人审批，下一个节点又不是会签节点，则需要选择下一个审批人
                if (nextApproveNode.getString("handleStrategy").equals("1")){
                    PageData pageData = new PageData();
                    pageData.put("approveNodeId",waitData.getString("approveNodeId"));
                    pageData.put("processInstId",waitData.getString("processInstId"));
                    //todo 查询未审批的人员待办信息
                    List<PageData> waitList = (List<PageData>) baseDao.findForList("FlowMapper.queryWaitDoneNoByUserId", waitData);
                    if (!CollectionUtils.isEmpty(waitList) && waitList.size() == 1){
                        //todo 查找下一个审批人
                        approvalUserList = getApprovalUserList(nextApproveNode);
        ...
        if (!CollectionUtils.isEmpty(approvalUserList)){
            //todo 查询审批人员的详细信息
            userList = (List<PageData>) baseDao.findForList("UserMapper.selectUserInfoByUserCode", approvalUserList);
            if(!CollectionUtils.isEmpty(userList)){
                //查询部门职务表
                List<PageData> postList = (List<PageData>) baseDao.findForList("UserMapper.queryAllPostData",approvalUserList);
                if(!CollectionUtils.isEmpty(postList)){
                    for(PageData data : userList){
                        String userCode = data.getString("userCode");
                        for(PageData postData : postList){
