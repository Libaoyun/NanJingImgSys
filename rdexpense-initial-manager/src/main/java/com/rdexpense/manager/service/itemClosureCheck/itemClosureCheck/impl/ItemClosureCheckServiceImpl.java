...
public class ItemClosureCheckServiceImpl implements ItemClosureCheckService {
    ...
    public List<PageData> queryProjectApplyList(PageData pd) {
        ...
        if (!CollectionUtils.isEmpty(pageDataList)){
            if (!pd.getString("menuFlag").isEmpty() && pd.getString("menuFlag").equals("1")){
                for (PageData pageData : pageDataList) {
                    pageData.put("startYear",pageData.getString("startYearOriginal"));
                    pageData.put("endYear",pageData.getString("endYearOriginal"));
                }
            }
        }
        return pageDataList;
    ...
    public List<PageData> queryApplyUserList(PageData pd) {
        ...
        //先获取研发项目人员变更的主表业务主键ID
        PageData pageData = (PageData) baseDao.findForObject("ItemClosureCheckMapper.queryItemChange", pd);
        if (pageData != null){
            pageDataList = (List<PageData>) baseDao.findForList("ItemClosureCheckMapper.queryItemChangeUserList", pageData);
            if (CollectionUtils.isEmpty(pageDataList)){
                //查询研发项目申请表下的人员信息
...
//        }
        ...
        if (pd.getString("id").isEmpty()){
            ...
            if (pd.getString("flag").isEmpty()){
                pd.put("processStatus", ConstantValUtil.APPROVAL_STATUS[0]);
                String serialNumber = SequenceUtil.generateSerialNo("XMYS");//生成流水号
                String businessId = "XMYS" + UUID.randomUUID().toString();//生成业务主键ID
    ...
    public HSSFWorkbook exportExcel(PageData pageData) {
        ...
        if (!CollectionUtils.isEmpty(checkInfoList)) {
            for (int i = 0; i < checkInfoList.size(); i++) {
                ...
                cell = row.createCell(j++);
                cell.setCellValue(pd.getString("nextApproveUserName"));
                cell.setCellStyle(styleCell);
                ...
                cell = row.createCell(j++);
                cell.setCellValue(pd.getString("checkDate"));
                cell.setCellStyle(styleCell);
    ...
    public void exportPDF(PageData data, Document document) throws Exception {
        ...
        String[] argArr1 = {
                ...
                "结题申报人",data.getString("creatorUser"),"申请评审日期",data.getString("checkDate"),
