...
public class Swagger2Configuration {
    ...
    //api文档地址 http://localhost:8066/rdexpense-initial/doc.html#/home
    @Bean(value = "公共接口")
    ...
    public Docket oneApi() {
        ...
        return new Docket(DocumentationType.SWAGGER_2)
                ...
                .groupName("1.系统基础接口")
                .select()
                .apis(Predicates.or(basePackage("com.rdexpense.manager.controller"),basePackage("com.imgSys.controller")
                ))
                .paths(Predicates.or(
                        ...
                        PathSelectors.ant("/imgFileInfoSubmit/**"), //患者基本信息管理
                        PathSelectors.ant("/patientImg/**") //患者采集单信息管理

                )) // 可以根据url路径设置哪些请求加入文档，忽略哪些请求
                /*.apis(Predicates.or(basePackage("com.imgSys.controller")
                ))
                .paths(Predicates.or(
                        PathSelectors.ant("/body-part/**"), //科室人体部位配置
                        PathSelectors.ant("/dig-item/**"), //科室项目诊疗配置
    ...
    public Docket progressReport() {
        ...
        return new Docket(DocumentationType.SWAGGER_2)
                ...
                .paths(Predicates.or(
                        ...
                        PathSelectors.ant("/attendance/**"),//人员考勤管理
                        PathSelectors.ant("/materialRequisition/**")//领料单
    ...
    public Docket applyApi() {
        ...
        return new Docket(DocumentationType.SWAGGER_2)
                ...
                .paths(Predicates.or(
                        //这里添加你需要展示的接口,同一包下根据url路径设置加入文档,忽略哪些文档
                        PathSelectors.ant("/equipmentDepreciationExpenses/**"),//设备折旧支出单管理
                        PathSelectors.ant("/equipmentLeaseSettlement/**"),//租赁设备结算管理
                        PathSelectors.ant("/itemChange/**"),//研发项目变更管理
    ...
    public Docket analysisApi() {
        ...
        return new Docket(DocumentationType.SWAGGER_2)
                .apiInfo(apiInfo())
                .groupName("3.研发项目统计分析")
                .select()
                .apis(Predicates.or(basePackage("com.rdexpense.manager.controller")
                ))
                .paths(Predicates.or(
                        //这里添加你需要展示的接口,同一包下根据url路径设置加入文档,忽略哪些文档
                        PathSelectors.ant("/BudgetAndPayContrast/**"),//预算与支出对比表
                        PathSelectors.ant("/CostAddDeductionDetailSummary/**"),//项目支出辅助账
                        PathSelectors.ant("/expensesBill/**"),//项目支出辅助账
                        PathSelectors.ant("/expensesBillSummary/**")//项目支出辅助账汇总表
