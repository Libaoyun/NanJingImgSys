...
public class BodyPartServiceImpl implements BodyPartService {
    ...
    /**
     * 树状查询所有部位结果(暂不包括示意图)（以一个起始节点作为开始）
     * @param rootSerialNum
     * @return
     */
    @Override
...
//    递归获取子部位
     ...
     */
    @Override
    public List<PageData> getStandardBodyPart(PageData pd) {
        List departments = Arrays.asList(pd.getString("departmentId").split(","));
        if (departments.size()>1){
            pd.put("departments", departments);
            pd.remove("departmentId");
        }
        List<PageData> list = (List<PageData>) baseDao.findForList("BodyPartMapper.getStandardBodyPart", pd);
